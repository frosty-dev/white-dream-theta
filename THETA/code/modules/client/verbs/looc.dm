/client/verb/looc(msg as text)
	...
	if(GLOB.say_disabled)	//смотрим лаги
	...
	if(!holder)
		...
		if(findtext(msg, "byond://"))
			to_chat(src, "<span class='bold'>ѕиарить чужие сервера запрещено!.</span>")
			log_admin("[key_name(src)] ѕыталс€ пропиарить другой сервер!: [msg]")
			message_admins("[key_name_admin(src)] ѕыталс€ прорекламить сервер в looc!: [msg]")
			return
