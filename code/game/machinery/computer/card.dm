...
/obj/machinery/computer/card/ui_interact(mob/user)
	...
	if (mode == 1) // accessing crew manifest
		...
		dat += {"<a href='?src=[REF(src)];choice=return'>Return</a>
		 || Confirm Identity:
		<a href='?src=[REF(src)];choice=scan'>[(scan ? rhtml_encode(scan.name) : "--------")]</a>
		...
		var/scan_name = scan ? rhtml_encode(scan.name) : "--------"
		var/target_name = modify ? rhtml_encode(modify.name) : "--------"
		var/target_owner = (modify && modify.registered_name) ? rhtml_encode(modify.registered_name) : "--------"
		var/target_rank = (modify && modify.assignment) ? rhtml_encode(modify.assignment) : "Unassigned"
