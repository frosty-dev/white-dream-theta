...
/obj/machinery/computer/card/ui_interact(mob/user)
	...
	if (mode == 1) // accessing crew manifest
		...
		var/target_rank
		if(modify)
			target_name = r_html_encode(modify.name)
		else
			target_name = "--------"
		if(modify && modify.registered_name)
			target_owner = r_html_encode(modify.registered_name)
		else
			target_owner = "--------"
		if(modify && modify.assignment)
			target_rank = r_html_encode(modify.assignment)
		else
			target_rank = "Unassigned"

		var/scan_name
		if(scan)
			scan_name = r_html_encode(scan.name)
		else
			scan_name = "--------"
