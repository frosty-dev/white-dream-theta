...
/obj/item/toy/gun/afterattack(atom/target as mob|obj|turf|area, mob/user, flag)
	...
	if (src.bullets < 1)
		user.show_message("<span class='warning'>*����*</span>", MSG_AUDIBLE)
		playsound(src, 'sound/weapons/gun_dry_fire.ogg', 30, TRUE)
		return
	playsound(user, 'sound/weapons/gunshot.ogg', 100, TRUE)
	src.bullets--
	user.visible_message("<span class='danger'><b>[user]</b> �������� �� <b>[src.name]</b> � <b>[target]</b>!</span>", \
						"<span class='danger'>�� ��������� �� <b>[src.name]</b> � <b>[target]</b>!</span>", \
