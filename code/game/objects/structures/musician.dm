...
/datum/song/Topic(href, href_list)
	...
	if(href_list["newsong"])
		...
		var/t = ""
		do
			t = r_html_encode(input(usr, "Please paste the entire song, formatted:", text("[]", name), t)  as message)
			if(!usr.canUseTopic(instrumentObj, BE_CLOSE, FALSE, NO_TK))
				return
	...
	if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
		...
		var/newline = r_html_encode(input("Enter your line: ", instrumentObj.name) as text|null)
		...
		var/content = r_html_encode(input("Enter your line: ", instrumentObj.name, lines[num]) as text|null)
