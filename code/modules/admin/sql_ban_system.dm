...
/proc/is_banned_from(player_ckey, roles)
	...
	if(C)
		...
		var/servport = sanitizeSQL(world.port)
		var/datum/DBQuery/query_check_ban = SSdbcore.NewQuery("SELECT 1 FROM [format_table_name("ban")] WHERE ckey = '[player_ckey]' AND WHERE server_port = '[servport]' AND role IN ('[sql_roles]') AND unbanned_datetime IS NULL AND (expiration_time IS NULL OR expiration_time > NOW())[admin_where]")
