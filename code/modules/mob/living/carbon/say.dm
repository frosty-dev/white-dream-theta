/mob/living/carbon/treat_message(message)
	...
	if(!T) //hoooooouaah!
		...
		var/regex/ru_tongueless_lower = new("\[„‰ÌÚÍËÂÁ]+", "„")
		var/regex/ru_tongueless_upper = new("\[√ƒÕ“ »≈«–]+", "„")
		if(copytext(message, 1, 2) != "*")
			message = tongueless_lower.Replace(message, pick("aa","oo","'"))
			message = tongueless_upper.Replace(message, pick("AA","OO","'"))
			message = ru_tongueless_lower.Replace(message, pick("‡‡","ÛÛ","˚˚"))
			message = ru_tongueless_upper.Replace(message, pick("¿¿","””","€€"))
	else
